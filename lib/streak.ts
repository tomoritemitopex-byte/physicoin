...
export function bumpStreak():number{ try{ const now=todayWAT(); const cur=getStreak(); const last=localStorage.getItem(K_LAST); if(last===now) return cur.decayed||cur.streak; // already bumped today
  ...
  localStorage.setItem(K_VAL,String(next)); localStorage.setItem(K_LAST,now); try{ new BroadcastChannel("physicoin_streak").postMessage({type:"bump", streak:next}); }catch{} return next; }catch{ return 1; } }
