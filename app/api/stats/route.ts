...
export async function GET() {
  ...
  try { await ensureAllTables(); } catch (e) { console.warn("[stats] ensure:", (e as Error).message); }
  ...
  const cnt = (i: number) => {
    const v = res[i];
    return v.status === "fulfilled" && Array.isArray(v.value) ? Number((v.value[0] as any)?.c ?? 0) : 0;
  };
  const rows = (i: number) => {
    const v = res[i];
    return v.status === "fulfilled" && Array.isArray(v.value) ? (v.value as any[]) : [];
  };
  const one = (i: number) => rows(i)[0] ?? null;

  const byStatusRows = rows(4);
  const byScopeRows = rows(5);
  const byVoteRows = rows(6);
  const miningAgg = one(7) ?? { c: 0, total: 0, avg: 0 };
  const userAgg = one(8) ?? { bal: 0, avg_auth: 0, max_auth: 0 };
  ...
  if (allFailed) {
    return NextResponse.json({
      ...
      metrics: { users:0, events:0, events_by_status:{}, events_by_scope:[], upcoming_events:0, verifications:0, verifications_by_vote:{}, mining_logs:0, mining_total_earned:0, mining_avg_earned:0, mines_last_24h:0, total_mining_balance:0, avg_authority_final:0, max_authority_final:0 },
  ...
  return NextResponse.json({
    ...
    metrics: {
      users: cnt(0), events: cnt(1),
      events_by_status: byStatus, events_by_scope: byScopeRows,
      upcoming_events: cnt(9),
      verifications: cnt(2), verifications_by_vote: byVote,
      mining_logs: cnt(3), mining_total_earned: Number(miningAgg.total), mining_avg_earned: Number(miningAgg.avg),
      mines_last_24h: cnt(10),
      total_mining_balance: Number(userAgg.bal), avg_authority_final: Number(userAgg.avg_auth), max_authority_final: Number(userAgg.max_auth),
    },
