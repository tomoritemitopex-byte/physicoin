  ...
  return (
    <div className="mx-auto flex max-w-[1280px] items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
      <div className="min-w-0">
        <h2 className="truncate text-[13px] font-semibold tracking-tight text-white sm:text-[15px]">
          {title}
        </h2>
        <ol className="mt-0.5 flex items-center gap-1.5 font-mono text-[11px] leading-none">
          ...
          {!isHome ? (
            ...
            <li className="text-slate-500">overview</li>
          )}
        </ol>
      </div>
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#070a12] text-slate-200 selection:bg-white selection:text-[#070a12]">
      {/* ambient — TailAdmin-inspired soft glows */}
      ...
      >
        <div className="mx-auto flex max-w-[1280px] items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
          ...
          {/* desktop nav — TailAdmin ComponentCard style pill */}
          <nav className="hidden items-center gap-1 rounded-full border border-white/[0.07] bg-white/[0.035] p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_4px_16px_rgba(0,0,0,0.2)] sm:flex">
            {TABS.map((t) => {
              ...
              return (
                <a
                  key={t.href}
                  href={t.href}
                  className={`relative rounded-full px-[14px] py-[6px] text-[13px] font-medium transition ${
                    active
                      ? "bg-white text-[#0a0f1e] shadow-[0_2px_10px_rgba(255,255,255,0.14),0_1px_0_rgba(255,255,255,0.6)_inset]"
            ...
            >
              ...
              <span
                className={`flex h-6 w-6 items-center justify-center rounded-full text-[11px] font-bold ${
                  isProfile ? "bg-[#070a12] text-white" : "bg-white text-[#0a0f1e]"
                }`}
              >
                ◯
              </span>
        ...
        {mobileOpen && (
          <div className="border-t border-white/[0.06] bg-[#070a12]/95 px-4 py-3 backdrop-blur sm:hidden">
            <nav className="grid grid-cols-2 gap-2">
                ...
                return (
                  <a
                    ...
                    className={`rounded-[12px] border px-3 py-2.5 text-center text-[13px] font-medium transition ${
                      active
                        ? "border-white bg-white text-[#070a12] shadow-[0_4px_16px_rgba(255,255,255,0.12)]"
              ...
              <a
                href="/app/profile"
                className={`col-span-2 rounded-[12px] border px-3 py-2.5 text-center text-[13px] font-medium transition ${
                  isProfile
      ...
      <footer className={isFullBleed ? "w-full border-t border-white/[0.04] px-4 py-6 sm:px-6 lg:px-8" : "mx-auto max-w-[1280px] border-t border-white/[0.04] px-4 py-6 sm:px-6 lg:px-8"}>
        <div className="flex flex-col items-center justify-between gap-3 sm:flex-row">
          ...
          <div className="flex items-center gap-2 font-mono text-[10.5px] text-slate-500">
            ...
            <a href="/app/verify" className="hover:text-slate-300 transition">
              Verify
            </a>
            <span className="opacity-30">·</span>
            <a href="/app/roadmap" className="hover:text-slate-300 transition">
              Roadmap
            </a>
          </div>
        </div>
