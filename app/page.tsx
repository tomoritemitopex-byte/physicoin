    ...
    <div className="min-h-screen bg-[var(--physi-bg)] text-[var(--physi-text)] landing-glow">
      <header className="border-b landing-rule">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-5 py-4 sm:px-8">
          <a href="/" className="flex items-center gap-3" aria-label="PhysiCoin home">
            <span className="grid h-9 w-9 place-items-center rounded-lg bg-[var(--physi-cyan)] font-black tracking-tight text-[#07111f]">P</span>
            <span className="font-mono text-sm font-bold tracking-[.18em]">PhysiCoin</span>
      ...
      <main className="mx-auto max-w-[1180px] px-5 sm:px-8">
        <section className="grid gap-12 py-14 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:py-24">
          <div>
            ...
            <p className="mt-7 max-w-[560px] text-base leading-7 text-slate-400 sm:text-lg">PhysiCoin turns the small campus updates that usually get lost in group chats into a visible, verifiable live feed.</p>
          ...
          <div><SignalRadar /><p className="mt-3 text-center font-mono text-[10px] uppercase tracking-[.12em] text-slate-500">green means go</p></div>
        </section>

        <section className="border-y landing-rule py-10"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="landing-kicker">The protocol</p><h2 className="mt-2 text-2xl font-bold tracking-tight sm:text-3xl">From rumour to signal.</h2></div><p className="max-w-sm text-sm leading-6 text-slate-400">No pretending the feed is official. No hiding uncertainty. PhysiCoin makes the strength of every update legible.</p></div><div className="mt-8 grid gap-3 md:grid-cols-3">{steps.map(([number, title, copy]) => <article key={number} className="landing-panel p-5"><span className="font-mono text-xs text-[var(--physi-cyan)]">/{number}</span><h3 className="mt-8 text-lg font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-400">{copy}</p></article>)}</div></section>
        ...
        <footer className="flex flex-wrap items-center justify-between gap-4 border-t landing-rule py-7 font-mono text-xs text-slate-500"><span>PhysiCoin / campus signal layer</span><span>$PHY has no cash value · <a href="/terms" className="text-slate-300 underline">terms</a></span></footer>
      </main>
    </div>
  );
}
